	...
	if open then 
		...
		while open do 
		   RageUI.IsVisible(mainMenu7,function() 
			...
			RageUI.Checkbox("Prendre votre service", nil, servicetaxi, {}, {
			...
			RageUI.Button("→ Annonces", nil, {RightLabel = "→→→"}, true , {
				onActive = function()
					RageUI.Info("Gestion taxi", {"Type : ~r~Annonces~w~"}, {})
				end, 
				onSelected = function()
				end
			}, subMenu8)
			...
			RageUI.Button("→ Intéractions", nil, {RightLabel = "→→→"}, true , {
				onActive = function()
					RageUI.Info("Gestion taxi", {"Type : ~r~Intéractions~w~"}, {})
				end, 
				onSelected = function()
				end
			}, subMenu9)
			...
			RageUI.IsVisible(subMenu8,function() 
			 ...
			 RageUI.Button("→ Ouverture", nil, {RightLabel = "→→→"}, true , {
				onActive = function()
					RageUI.Info("Gestion taxi", {"Type : ~g~Ouvert~w~"}, {})
				end, 
				onSelected = function()
					ESX.ShowAdvancedNotification('taxi', '~r~Annonce', '~g~Le taxi est désormais ouvert vient te faire plaisir chez nous !', 'CHAR_FLOYD', 8)
				end
			})

			RageUI.Button("→ Fermeture", nil, {RightLabel = "→→→"}, true , {
				onActive = function()
					RageUI.Info("Gestion taxi", {"Type : ~r~Fermer~w~"}, {})
				end, 
				onSelected = function()
					ESX.ShowAdvancedNotification('taxi', '~r~Annonce', '~r~Le taxi est désormais fermé à plus tard !', 'CHAR_FLOYD', 8)
				end
			})

			RageUI.Button("→ Recrutement", nil, {RightLabel = "→→→"}, true , {
				onActive = function()
					RageUI.Info("Gestion taxi", {"Type : ~y~Recrutement~w~"}, {})
				end, 
				onSelected = function()
				ESX.ShowAdvancedNotification('taxi', '~r~Annonce', '~y~Recrutement en cours, rendez-vous au taxi!', 'CHAR_FLOYD', 8)
				end
			})
				...
				RageUI.IsVisible(subMenu9,function()
					...
					RageUI.Button("→ Missions", nil, {RightLabel = "→→→"}, true , {
						onActive = function()
							RageUI.Info("Gestion taxi", {"Type : ~r~Mission PNG~w~", "Utilité : ~r~Gagner de l'argent sans joueurs~w~"}, {})
						end, 
						onSelected = function()
						StartTaxiMission()
						end
					...
					RageUI.Button("→ Facturer", nil, {RightLabel = "→→→"}, true , {
						onActive = function()
							RageUI.Info("Gestion taxi", {"Type : ~r~Facturation~w~", "Utilité : ~r~Facturer Joueur ~w~"}, {})
						end,
						onSelected = function()
...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName('~r~Entreprises ~w~Taxi') -- Nom du blip
