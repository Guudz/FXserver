...
AddEventHandler('Ouvre:taxi', function()
	...
	for i=1, #xPlayers, 1 do
		...
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Taxi', '~p~Annonce', 'Le taxi est désormais ~g~Ouvert~s~!',  8)
	end
end)
...
AddEventHandler('Ferme:taxi', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Taxi', '~p~Annonce', 'Le taxi est désormais ~r~Fermer~s~!',  8)
	end
end)
