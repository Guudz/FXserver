    ...
    if xPlayer.getGroup() ~= 'user' then
        ...
        if TargetPlayer then
            ...
            if tonumber(JailTime[TargetPlayer.source].time) >= 1 then
                TriggerClientEvent(Config.ESX.."esx:showNotification", source, 'Le joueur est déjà en jail pendant: ~g~'.. JailTime[TargetPlayer.source].time.. ' ~w~minutes')
            else
                ...
                TriggerClientEvent(Config.ESX.."esx:showNotification", TargetPlayer.source, 'Vous avez jail été jail ~w~pendant ~g~'..args[2].. ' ~w~minute')
            ...
            if tonumber(JailTime[TargetPlayer.source].time) >= 0 then
                if Config.StaffOnlyUnjailPlayerJail then 
                    if JailTime[TargetPlayer.source].staffname == xPlayer.getName() then 
                        ...
                        TriggerClientEvent(Config.ESX.."esx:showNotification", source, 'Le joueur ~g~'..GetPlayerName(xPlayer.source)..' ~w~à été unjail')
                        ...
                        xPlayer.showNotification("Vous ne pouvez pas sortir ce joueur de prison car vous n'étes pas l'auteur du jail !")
                    end
                else
                    JailTime[TargetPlayer.source].time = 0
                    TriggerClientEvent(Config.ESX.."esx:showNotification", source, 'Le joueur ~g~'..GetPlayerName(xPlayer.source)..' ~w~à été unjail')
