...
RegisterNetEvent("ronflex:addtenueitem", function(label, skin)
    ...
    if Account == 'nomoney' then
        xPlayer.showNotification('~b~GreenLife ~w~~n~Vous n\'avez pas assez d\'argent sur vous')
    else
        ...
        xPlayer.showNotification('~b~GreenLife~w~~n~Vous avez crée une tenue (~r~'..label..'~w~)')
...
RegisterNetEvent("ronflex:paidaccesoires", function(type, name, skin)
    ...
    if xPlayer.getAccount('cash').money >= 300 then 
        ...
        TriggerClientEvent("ronflex:recieveclientsidevetement", xPlayer.source, ClothesPlayer[xPlayer.identifier])
        xPlayer.showNotification("~b~GreenLife~s~~n~Vous venez d'acheter un "..type.."")
    else
        xPlayer.showNotification("~b~GreenLife~s~~n~Vous n'avez pas les fonds nécéssaires")
    end
...
RegisterNetEvent('ronflex:donnertenue', function(player, id)
    ...
    if tPlayer ~= nil then
        if ClothesPlayer[xPlayer.identifier][id] then
            ...
            xPlayer.showNotification("~b~GreenLife~s~~n~ Vous avez donné votre tenue ")
            tPlayer.showNotification("~b~GreenLife~s~~n~ Vous avez reçu une tenue ")
...
RegisterNetEvent('ewen:RenameTenue', function(id, NewLabel)
    ...
    if ClothesPlayer[xPlayer.identifier][id] then
        if ClothesPlayer[xPlayer.identifier][id].identifier == xPlayer.identifier then
            xPlayer.showNotification('~b~GreenLife~w~~n~Vous avez renommer votre tenue (~r~'..ClothesPlayer[xPlayer.identifier][id].label..'~w~)')
...
RegisterNetEvent('ronflex:deletetenue', function(id, NewLabel)
    ...
    if ClothesPlayer[xPlayer.identifier][id] then
        if ClothesPlayer[xPlayer.identifier][id].identifier == xPlayer.identifier then
            xPlayer.showNotification('~b~GreenLife~w~~n~Vous avez supprimer votre tenue (~r~'..ClothesPlayer[xPlayer.identifier][id].label..'~w~)')
            ClothesPlayer[xPlayer.identifier][id] = nil
            TriggerClientEvent("ronflex:recieveclientsidevetement", xPlayer.source, ClothesPlayer[xPlayer.identifier])
    ...
    if ClothesPlayer[xPlayer.identifier][id] then
        if ClothesPlayer[xPlayer.identifier][id].identifier == xPlayer.identifier then
            if typee == "equip" then 
                ...
                xPlayer.showNotification('~b~GreenLife~w~~n~Vous avez équiper votre tenue (~r~'..ClothesPlayer[xPlayer.identifier][id].label..'~w~)')
                TriggerClientEvent("ronflex:recieveclientsidevetement", xPlayer.source, ClothesPlayer[xPlayer.identifier])
            ...
            elseif typee == "deposit" then 
                ...
                xPlayer.showNotification('~r~SeaLife~w~~n~Vous avez déposer votre tenue (~r~'..ClothesPlayer[xPlayer.identifier][id].label..'~w~)')
                TriggerClientEvent("ronflex:recieveclientsidevetement", xPlayer.source, ClothesPlayer[xPlayer.identifier])
            end
        else
            DropPlayer(source, 'Mhh c\'est chaud c\'que t\'essaie de faire')
...
ESX.RegisterServerCallback("ronflex:cbmoneytenue", function(source, cb)
    ...
    if Account == "nomoney" then 
        ...
        cb(true)
        xPlayer.removeAccountMoney(Account, 500)
    end
end)