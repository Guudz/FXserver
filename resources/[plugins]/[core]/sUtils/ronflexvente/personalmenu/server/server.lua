...
RegisterNetEvent('ewen:ChangeWeightInventory',function(NewMaxWeight)
    local SECURITY = NewMaxWeight == 40 and true or NewMaxWeight == 60 and true or false

    if SECURITY then
        local xPlayer = ESX.GetPlayerFromId(source)
        xPlayer.setMaxWeight(NewMaxWeight)
        xPlayer.showNotification('~r~LystyLife ~w~~n~Le poids de votre inventaire a été changé à ~r~'..NewMaxWeight..'~w~kg.')
    else
