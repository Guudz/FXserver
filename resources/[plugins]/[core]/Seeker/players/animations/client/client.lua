...
local function AnimShared(Infos, ListId)
    ...
    if target then
        LystyLifeClientUtils.toServer("ServerEmoteRequest", GetPlayerServerId(target), ListId, Infos[3])
        ShowNotification("Demande envoyée à ~r~" .. GetPlayerName(target) .. " ~w~(~r~" .. Infos[3] .. "~w~)")
    else
        ShowNotification("Personne assez proche")
    end
end

LystyLife.netRegisterAndHandle("ClientEmoteRequestReceive", function(emoteanim, emotename)
    PlaySound(-1, "NAV", "HUD_AMMO_SHOP_SOUNDSET", 0, 0, 1)
    ShowNotification("~y~Y~w~ accepter, ~r~L~w~ refuser (~r~" .. emotename .."~w~)")
    local isRequestAnim = 1
    while isRequestAnim <= 100 do
        Citizen.Wait(5)
        isRequestAnim = isRequestAnim + 1
        if IsControlJustPressed(1, 246) then
            LystyLifeClientUtils.toServer("ServerValidEmote", GetPlayerServerId(GetClosestPlayerAnim(3)), emoteanim)
...
LystyLife.netRegisterAndHandle("SyncPlayEmote", function(emote, sourceP)
...
RegisterKeyMapping("emotemenu", "Ouvrir le menu d'animations", "keyboard", "K")

RegisterCommand("stop_animation", function()
    StopAnimation()
end, false)
RegisterKeyMapping("stop_animation", "Arreter une animation", "keyboard", "x")
...
LystyLife.netRegisterAndHandle('onResourceStop', function(resource)
