...
-- GENERAL --
Config.webhook = 'https://discord.com/api/webhooks/914151277586427995/sEvrdZrOU8cVlT2-yJtgXKD3DhAT3ec9S5H6M2kFRbxTXwnAAEswXuz__9p-X6bgiqy8'
	...
	IsBanned(licenseid, function(isBanned, banData)
		if isBanned then
			if tonumber(banData.permanent) == 1 then
				deferrals.done(('Vous êtes banni de '.. GetConvar("servername", "LystyLife Collective") ..'\nRaison : %s\nTemps Restant : Permanent\nAuteur : %s'):format(banData.reason, banData.sourceName))
...
AddEventHandler('BanSql:ICheatClient', function(reason)
	...
	if _source then
		...
		if name then
			...
			AddBan(_source, licenseid, playerip, name, 'Anti-Cheat LystyLife', 0, reason, 1)
			DropPlayer(_source, ('Vous êtes banni de '.. GetConvar("servername", "LystyLife Collective") ..'\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat '.. GetConvar("servername", "LystyLife Collective") ..''):format(reason))
		end
	else
		print('BanSql Error : Anti-Cheat '.. GetConvar("servername", "LystyLife Collective") ..' have received invalid id.')
	end
end)
...
AddEventHandler('BanSql:ICheatServer', function(target, reason)
	...
	if reason == nil then
		reason = 'Cheat/Autre - Ban par un Admin'
	end
	...
	if target then
		...
		if name then
			...
			AddBan(target, licenseid, playerip, name, 'Anti-Cheat '.. GetConvar("servername", "LystyLife Collective") ..'', 0, reason, 1)
			DropPlayer(target, ('Vous êtes banni de '.. GetConvar("servername", "LystyLife Collective") ..'\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat '.. GetConvar("servername", "LystyLife Collective") ..''):format(reason))
		end
	else
