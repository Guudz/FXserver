...
AddEventHandler('esx:playerLoaded', function(source)
        ...
        MySQL.Async.fetchScalar('SELECT expiration FROM tebex_accounts WHERE steam = @steam', {
            ...
            if tonumber(result) ~= 157 then
                if tonumber(result) <= os.time() then
                    xPlayer.showNotification('~r~LystyLife ~w~~n~Votre VIP ['..labeltext..'] Ã  expirer')
                    MySQL.Async.execute('UPDATE tebex_accounts SET `vip` = @vip, `expiration` = @expiration WHERE steam = @steam', {
                        ['@steam'] = steam,
                        ['@vip'] = 0,
                        ['@expiration'] = 0,
                    })
                    LystyLifeServerUtils.toClient('LystyLifeVIP:updateVip', source, 0)
                else
                    ...
                    xPlayer.showNotification('~r~LystyLife ~w~~n~Votre VIP : ['..labeltext..']\nPrendra fin dans ~r~'..math.floor(day).. ' ~w~jours, ~r~'..txthrs.."~w~ heure(s).")
                    LystyLifeServerUtils.toClient('LystyLifeVIP:updateVip', source, account.vip)
                end
            else
                xPlayer.showNotification('~r~LystyLife ~w~~n~Votre VIP est ~r~Permanent')
                LystyLifeServerUtils.toClient('LystyLifeVIP:updateVip', source, account.vip)
            end
        end)
    else
    ...
    LystyLifeServerUtils.toClient('LystyLifeVIP:updatePoints', source, account.points)
end)
LystyLife.newThread(function()
    while true do
        Citizen.Wait(2500)
...
function Tebex:tickShareAccounts()
    ...
    TriggerEvent('LystyLifeVIP:shareAccounts', data)
    LystyLifeServerUtils.toClient('LystyLifeVIP:updatePlayerLystyLifeVIP', -1, data)
        ...
        Switch(command, {
            ['addVip'] = function()
                ...
                MySQL.Async.execute('UPDATE tebex_accounts SET `vip` = @vip, `expiration` = @expiration WHERE steam = @steam', {
                    ...
                    account.vip = rank
                    if source ~= nil then
                        LystyLifeServerUtils.toClient('LystyLifeVIP:updateVip', source, rank)
                    end
                end)
            end,
...
ESX.RegisterServerCallback("LystyLifeVIP:getVip", function(source, cb)
    if source then
        local xPlayer = ESX.GetPlayerFromId(source)
        if xPlayer then
...
RegisterCommand('addVip', function(source, args)
    ...
    LystyLife.newThread(function()
        ...
        if vipRank == 1 then vipLabel = "VIP ~y~Gold~s~." money = 1000000 elseif vipRank == 2 then vipLabel = "VIP ~r~Diamond~s~." money = 3000000 end
        ...
        for i=1, #xPlayers, 1 do
            ...
            if fivem == fivemPlayer then
                ...
                LystyLifeServerUtils.toClient('LystyLifeVIP:updateVip', xPlayer.source, vipRank)
    ...
    LystyLife.newThread(function()
        ...
        for i=1, #xPlayers, 1 do
            ...
            if fivem == fivemPlayer then
                ...
                LystyLifeServerUtils.toClient('LystyLifeVIP:updateVip', xPlayer.source, 0)
            end
        end
    end)
end)
...
RegisterCommand('viptime', function(source)
    ...
    if account.vip ~= 0 then
        ...
        xPlayer.showNotification('~r~LystyLife~w~~n~Vous n\'avez pas de VIP')
    end
end)