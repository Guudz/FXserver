LystyLife.netRegisterAndHandle('esx_truck_inventory:putItem', function(itemName, type, count, plate)
		...
		if type == 'item_standard' then
			...
			if playerItem.count >= count and count > 0 then
				TriggerEvent('esx_addoninventory:getInventory', 'trunk', plate, function(inventory)
					...
					TriggerClientEvent('esx:showNotification', xPlayer.source, 'Vous avez déposé ~y~x' .. count .. '~s~ ~r~' .. ESX.GetItem(itemName).label .. '~s~')
				end)
			else
		...
		elseif type == 'item_weapon' then
			...
			if xPlayer.hasWeapon(weaponName) then
				TriggerEvent('esx_datastore:getDataStore', 'trunk', plate, function(store)
					...
					TriggerClientEvent('esx:showNotification', xPlayer.source, 'Vous avez déposé ~y~x' .. count .. '~s~ ~r~' .. ESX.GetWeaponLabel(itemName) .. '~s~')
				end)
			else
...
LystyLife.netRegisterAndHandle('esx_truck_inventory:getItem', function(itemName, type, count, plate)
	local xPlayer = ESX.GetPlayerFromId(source)
			...
			TriggerEvent('esx_addoninventory:getInventory', 'trunk', plate, function(inventory)
				...
				if inventoryItem.count >= count and count > 0 then
					if xPlayer.canCarryItem(itemName, count) then
						...
						TriggerClientEvent('esx:showNotification', xPlayer.source, 'Vous avez pris ~y~x' .. count .. '~s~ ~r~' .. inventoryItem.label .. '~s~')
					else
				...
				if not xPlayer.hasWeapon(weaponName) then
					...
					for i = 1, #weapons, 1 do
						if weapons[i].name == weaponName and weapons[i].ammo == count then
							...
							TriggerClientEvent('esx:showNotification', xPlayer.source, 'Vous avez pris ~y~x' .. count .. '~s~ ~r~' .. ESX.GetWeaponLabel(itemName) .. '~s~')
							break
						end
					end
				else
