...
function OpenNewCoffre(vehicle)
    local menu = RageUI.CreateMenu("Coffre Véhicule", "Actions disponible")
...
function OpenPlayerCoffre(vehicle)
    ...
    local inventaireplayer = RageUI.CreateMenu('Coffre Véhicule', "Inventaire")
            ...
            for k,v in pairs(data.inventory) do
                RageUI.Button(v.label, nil, { RightLabel = "Quantité(e)s : ~r~"..v.count }, true, {
                    onSelected = function()
						...
						LystyLifeClientUtils.toServer('esx_truck_inventory:putItem', v.name, 'item_standard', tonumber(quantity), GetVehicleNumberPlateText(vehicle))
                        RageUI.CloseAll()
                    end
                })
            end
                ...
                RageUI.Button(v.label, nil, { RightLabel = "Munitions : ~r~"..v.ammo }, true, {
                    onSelected = function()
                        if WeaponBlacklist[v.name] == nil then
                            LystyLifeClientUtils.toServer('esx_truck_inventory:putItem', v.name, 'item_weapon', v.ammo, GetVehicleNumberPlateText(vehicle))
                            RageUI.CloseAll()
                        else
                            ESX.ShowNotification('~r~LystyLife ~w~~n~Vous ne pouvez pas déposer les armes boutique dans les coffres')
                        end
                    end
                })
            end
        ...
        RageUI.IsVisible(inventaireplayer, function()
                ...
                if v.count ~= 0 then
                    RageUI.Button(v.label, nil, { RightLabel = "Quantité(e)s : ~r~"..v.count }, true, {
                        onSelected = function()
                            ...
                            LystyLifeClientUtils.toServer('esx_truck_inventory:getItem', v.name, 'item_standard', tonumber(quantity), GetVehicleNumberPlateText(vehicle))
                            RageUI.CloseAll()
                        end
                    })
                end
            ...
            for k,v in pairs(data2.weapons) do
                RageUI.Button(ESX.GetWeaponLabel(v.name), nil, { RightLabel = "Munitions : ~r~"..v.ammo }, true, {
                    onSelected = function()
						LystyLifeClientUtils.toServer('esx_truck_inventory:getItem', v.name, 'item_weapon', v.ammo, GetVehicleNumberPlateText(vehicle))
                        RageUI.CloseAll()
                    end
                })
            end
