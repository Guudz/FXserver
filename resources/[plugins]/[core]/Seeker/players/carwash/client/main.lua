...
local function commencercarwash()
	...
	Wait(1000)
	fincarwashprops()
	--ShowAboveRadarMessage("~r~CarWash\n~s~Vous avez lavé votre ~r~"..GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(pVeh))).."~s~.")
	...
	LystyLifeClientUtils.toServer('clp_carwash:removemoney', price)
	ClearPedTasks(pPed)
end;
		...
		for _,v in pairs(poscar) do
			if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
				...
				if Vdist2(GetEntityCoords(PlayerPedId(), false), v) < 7 then
					attente = 1
					DrawText3D(v.x, v.y, v.z+0.2, "Appuyez sur ~r~E ~w~pour ~r~laver votre véhicule~w~.", 9)
...
function openWarcash()
	...
	while menu do
		...
		RageUI.IsVisible(menu, function()
            RageUI.Button('Néttoyer sa Voiture ( 20$ )', nil, {}, true, {onSelected = function()
                LystyLifeClientUtils.toServer('framework:carwash')
            end})
        end, function()
        end)
...
LystyLife.netRegisterAndHandle('framework:cleanvehicle', function()
