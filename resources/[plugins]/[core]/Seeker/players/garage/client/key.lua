	...
	ESX.TriggerServerCallback('Core:requestPlayerCars', function(gotkey)
		if gotkey then
			...
			if locked == 1 or locked == 0 then
				...
				ESX.ShowAdvancedNotification('MercuryLife', '~y~Clés', "Vous avez ~r~fermé~s~ le véhicule.", 'CHAR_SEALIFE', 7)
			elseif locked == 2 then
				SetVehicleDoorsLocked(vehicle, 1)
				PlayVehicleDoorOpenSound(vehicle, 0)
				ESX.ShowAdvancedNotification('MercuryLife', '~y~Clés', "Vous avez ~r~ouvert~s~ le véhicule.", 'CHAR_SEALIFE', 7)
			end
		else
			ESX.ShowAdvancedNotification('MercuryLife', '~y~Clés', "~r~Vous n'avez pas les clés de ce véhicule.", 'CHAR_SEALIFE', 7)
		end
