            ...
            if tostring(TypeGarage) == 'car' then
                ...
                RageUI.Separator('Vos Véhicule(s).')
                    for k,v in pairs(localCarsOwned) do
                        if v.type == 'car' then
                            if v.label ~= nil then 
                                RageUI.Button('['..GetLabelText(GetDisplayNameFromVehicleModel(v.vehicle.model))..'] - '..v.label, (v.state == true and "" or "Se véhicule se trouve à la Fourière !"), {RightBadge = RageUI.BadgeStyle.Tick}, v.state, {
                                    onActive = function()
                    ...
                    for k,v in pairs(localCarsOwned) do
                        if v.type == 'car' and not v.state then
                            if v.label ~= nil then 
                                RageUI.Button(GetLabelText(GetDisplayNameFromVehicleModel(v.vehicle.model)), "", {}, not v.state, {
                                    onActive = function()
