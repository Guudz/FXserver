...
function Fleeca:AnimStartFin(PEqsd)
    ...
    LystyLife.toInternal("cFleeca:HackingMinigame", 2)
...
function Fleeca:FinFleeca()
    ...
    Fleeca:AnimStartFin()
    LystyLife.toInternal("cFleeca:clientBankHeist", 3, fMoneyWin)
end
...
function Fleeca:StartScalform()
    ...
    LystyLifeClientUtils.toServer("clp:removeitem", "drill", 0)
...
LystyLife.netRegisterAndHandle("cFleeca:OpenDoor", function(prop, doorpos)
    if prop ~= nil then 
        Citizen.Wait(800)
        local count = 0
...
LystyLife.netRegisterAndHandle("cFleeca:CloseDoor", function(prop, doorpos)
...
function Fleeca:OpenDoorFl(id)
    ...
    if getObjdoor and DoesEntityExist(getObjdoor) then
        ...
        LystyLifeClientUtils.toServer("cFleeca:OpenDoor", getObjdoor, doorpos)
    end
end
function Fleeca:AnimPhone1(Xfn)
    ...
    LystyLife.toInternal("cFleeca:clientBankHeist", 2, Fleeca.nFleeca)
    Fleeca.moove = 1
end
        ...
        while Fleeca.nFleeca ~= 0 do
            ...
            if Fleeca.moove == 1 then
                ...
                if disTance < 1.3 then
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour utiliser votre ~r~perceuse.")
...
function Fleeca:StartFleecaHeist()
    ...
    LystyLife.toInternal("cFleeca:clientBankHeist", 1, {id = FleecaIn})
end

LystyLife.netRegisterAndHandle("cFleeca:StartDrill", function()
    Fleeca:StartFleecaHeist()
end)

LystyLife.netRegisterAndHandle("cFleeca:BinginDrill", function()
    local pos = Fleeca.posFleeca[Fleeca.nFleeca.id].rewardPos
    LystyLifeClientUtils.toServer("sPolice:enterAppel", pos.x, pos.y, pos.z, "Braquage", "Notre Fleeca se fait actuellement braquer !")
    Fleeca:StartCallScal()
    LystyLifeClientUtils.toServer("cFleeca:SetCooldown", Fleeca.nFleeca.id)
end)

LystyLife.netRegisterAndHandle("cFleeca:clientBankHeist", function(bool, _)
    if bool == 1 then
        Fleeca.nFleeca = _;
        LystyLifeClientUtils.toServer("cFleeca:GetCoolDown", Fleeca.nFleeca.id)
    elseif bool == 2 then
        if _ then
            Fleeca:OpenDoorFl(_.id)
        end
    elseif bool == 3 then
        LystyLifeClientUtils.toServer("cFleeca:GiveMoney", token)
        Wait(15*60*1000)
        if Fleeca.ObjectDoorid ~= nil and Fleeca.ObjectDooridPos ~= nil then 
            LystyLifeClientUtils.toServer("cFleeca:CloseDoor", Fleeca.ObjectDoorid, Fleeca.ObjectDooridPos)
        end
    end
end)