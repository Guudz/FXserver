...
OpenVetementShop = function()
    local mainvet = RageUI.CreateMenu("", "Bienvenue sur AndarisRP")
    ...
    while mainvet do 
        ...
        RageUI.IsVisible(mainvet, function()
            ...
            RageUI.Button("Nos Hauts", nil, {RightLabel = "→"}, true, {}, haut)

            RageUI.Button("Nos Bas", nil, {RightLabel = "→"}, true, {}, bas)

            RageUI.Separator("")

            RageUI.Button("Vos Tenues", nil, {RightLabel = "→"}, true, {}, garderobe)

            RageUI.Button("> Valider mon Panier", "Prix : 750.0 $", {RightBadge = RageUI.BadgeStyle.Alert, Color = {BackgroundColor = RageUI.ItemsColour.Blue}}, true, {
                onSelected = function()
            ...
            if ClothesPlayer ~= nil then 
                for k, v in pairs(ClothesPlayer) do 
                    if v.equip == "n" and v.type == "vetement" then 
                        NoTenueDispo = true
                        RageUI.List(v.label, {"Equiper", "Renomer", "Supprimer"}, Vetement.IndexGardeRobe, nil, {}, true, {
