...
RegisterNetEvent("ronflex:addtenueitem", function(label, skin)
    ...
    if NumberCount+1 > NumberTenueAutorized then 
        ...
        local Account = xPlayer.getAccount('cash').money >= 100 and 'money' or xPlayer.getAccount('bank').money >= 100 and 'bank' or 'nomoney'
        if Account == 'nomoney' then
            ...
            xPlayer.showNotification('Vous avez crée une tenue (~b~'..label..'~w~)')
...
RegisterNetEvent('ewen:RenameTenue', function(id, NewLabel)
    ...
    if ClothesPlayer[xPlayer.identifier][id] then
        if ClothesPlayer[xPlayer.identifier][id].identifier == xPlayer.identifier then
            xPlayer.showNotification('Vous avez renommer votre tenue (~b~'..ClothesPlayer[xPlayer.identifier][id].label..'~w~)')
...
RegisterNetEvent('ronflex:deletetenue', function(id, NewLabel)
    ...
    if ClothesPlayer[xPlayer.identifier][id] then
        if ClothesPlayer[xPlayer.identifier][id].identifier == xPlayer.identifier then
            xPlayer.showNotification('Vous avez supprimer votre tenue (~b~'..ClothesPlayer[xPlayer.identifier][id].label..'~w~)')
            ClothesPlayer[xPlayer.identifier][id] = nil
            TriggerClientEvent("ronflex:recieveclientsidevetement", xPlayer.source, ClothesPlayer[xPlayer.identifier])
    ...
    if ClothesPlayer[xPlayer.identifier][id] then
        if ClothesPlayer[xPlayer.identifier][id].identifier == xPlayer.identifier then
            if typee == "equip" then 
                ...
                xPlayer.showNotification('Vous avez équiper votre tenue (~b~'..ClothesPlayer[xPlayer.identifier][id].label..'~w~)')
                TriggerClientEvent("ronflex:recieveclientsidevetement", xPlayer.source, ClothesPlayer[xPlayer.identifier])
            ...
            elseif typee == "deposit" then 
                ...
                xPlayer.showNotification('Vous avez déposer votre tenue (~b~'..ClothesPlayer[xPlayer.identifier][id].label..'~w~)')
                TriggerClientEvent("ronflex:recieveclientsidevetement", xPlayer.source, ClothesPlayer[xPlayer.identifier])
            end
        else
            DropPlayer(source, 'Mhh c\'est chaud c\'que t\'essaie de faire')
