    ...
    while true do
        ...
        if dist < 150 then
            if IsPedSittingInVehicle(PlayerPedId(), KartingVehicle) then
                ...
                ESX.ShowNotification('~r~LystyLife ~w~~n~Vous avez quitté la zone du karting, Votre véhicule à été saisie')
            end
        end
        if dist2 < 50 then
            if IsPedSittingInVehicle(PlayerPedId(), KartingVehicle) then
...
function OpenMenuPlayerKarting()
    local menu = RageUI.CreateMenu("Karting", "Activité Karting")
    ...
    while menu do
        ...
        if not ActivityKarting then
            RageUI.Separator('~r~LystyLife ~w~Activité - Karting')
            RageUI.Separator('Un kart vous sera fourni')
            RageUI.Button('Commencer l\'activité', nil, {RightLabel = '~r~60$'}, true, {
                onSelected = function()
                    ActivityKarting = true
                    ESX.Game.SpawnVehicle("kart", vector3(-153.08323669434,-2147.4096679688,16.705001831055), nil, function(vehicle)
            ...
            RageUI.Separator('LystyLife Activité - Karting')
