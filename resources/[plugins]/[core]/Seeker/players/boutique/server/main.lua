                ...
                if (current >= price) then
                    ...
                    if CASHOUT[xPlayer.identifier] ~= nil then 
                        if CASHOUT[xPlayer.identifier] + price >= 5000 then
                            ...
                            xPlayer.showAdvancedNotification('Boite Mail', 'Boutique LystyLife', 'Il vous reste '..CASHOUT[xPlayer.identifier]..'/5000 points à utiliser\nAvant de toucher votre bonus fidélité !', 'CHAR_MP_FM_CONTACT', 2)
                        end
                    end
                    onAccepted();
                else
...
RegisterCommand('addPoints', function(source, args)
    if source == 0 then
        ...
        if (identifier['fivem']) then
            ...
            LiteMySQL:Insert('tebex_players_wallet', {
                identifiers = after,
                transaction = 'Ajout de LystyLifeCoins via la console',
                price = 0,
                currency = 'Points',
                points = args[2],
            });
          --  MomoLogs('https://discord.com/api/webhooks/890390456863698965/wrtf8uD815xJAuAjZoezVAxbEVh1WSvZZR5h9qSlBINfL0gUX_5nw3VcUhfbl7VGjgph', "Boutique - GiveCoins","**CONSOLE** vien de GIVE des Coins a :\n**License** : "..Target.."\nNombre de Coins : "..args[2].."", 56108)
            Target.showNotification('~r~LystyLife ~w~~n~Vous avez reçu ~r~'..args[2].. ' ~w~LystyLifeCoins')
        else
        ...
        if xPlayer.getGroup() == 'fondateur' then
            ...
            if (identifier['fivem']) then
                ...
                LiteMySQL:Insert('tebex_players_wallet', {
                    identifiers = after,
                    transaction = 'Ajout de LystyLifeCoins par '..GetPlayerName(source),
                    price = 0,
                    currency = 'Points',
                    points = args[2],
                });
              --  MomoLogs('https://discord.com/api/webhooks/890390456863698965/wrtf8uD815xJAuAjZoezVAxbEVh1WSvZZR5h9qSlBINfL0gUX_5nw3VcUhfbl7VGjgph', "Boutique - GiveCoins","**"..GetPlayerName(source).."** vien de GIVE des Coins a :\n**Nom** : "..Target.."\nNombre de Coins : "..args[2].."", 56108)
                Target.showNotification('~r~LystyLife ~w~~n~Vous avez reçu ~r~'..args[2].. ' ~w~LystyLifeCoins')
            else
                xPlayer.showNotification('~r~LystyLife ~w~~n~Le joueur n\'as aucun compte FiveM lier')
            end
        end
    end
end)
    ...
    if SecurityVehicles[model].model == model and SecurityVehicles[model].price == price then
        local xPlayer = ESX.GetPlayerFromId(source)
            OnProcessCheckout(source, SecurityVehicles[model].price, string.format("Achat de : %s", label), function()
                ...
                xPlayer.showNotification("~r~LystyLife~w~~n~Vous avez acheter : " .. label .. " sur la boutique !")
...
AddEventHandler('aBoutique:Entreprise', function()
    ...
    OnProcessCheckout(source, 5000, string.format("Achat de Pack Entreprise"), function()
        xPlayer.showNotification('~r~LystyLife~w~~n~Vous avez acquis le Pack Entreprise pour 5000 ~y~LystyLifeCoins\n~w~Faites un ticket sur Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter un ***Pack Entreprise***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [5000] LystyLifeCoins', 56108)       
    end, function()
        return
    end)
end)
...
AddEventHandler('aBoutique:Illegal', function()
    ...
    OnProcessCheckout(source, 5000, string.format("Achat de Pack Illégal"), function()
        xPlayer.showNotification('~r~LystyLife~w~~n~Vous avez acquis le Pack Illégal pour 5000 ~y~LystyLifeCoins\n~w~Faites un ticket sur Discord')
...
AddEventHandler('aBoutique:BuyCustomMax', function(vehicle)
    ...
    OnProcessCheckout(source, 1000, string.format("Achat de Pack Customisation"), function()
        ...
        xPlayer.showNotification('~r~LystyLife~w~~n~Vous avez acquis le Pack Customisation pour 1000 ~y~LystyLifeCoins')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter un ***Pack Customisation***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [1000] LystyLifeCoins', 56108)    
    end, function()
        return
    end)
end)
...
AddEventHandler('aBoutique:CircuitDrogue', function(vehicle)
    ...
    OnProcessCheckout(source, 5000, string.format("Achat Circuit de drogue personnalisé"), function()
        xPlayer.showNotification('~r~LystyLife~w~~n~Vous avez acquis un Circuit de drogue personnalisé our 5000 ~y~LystyLifeCoins\n~w~Faites un ticket sur Discord')
...
AddEventHandler('aBoutique:IllegalWeapon', function(vehicle)
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    OnProcessCheckout(source, 7500, string.format("Achat Vente d\'arme Illégal"), function()
        xPlayer.showNotification('~r~LystyLife~w~~n~Vous avez acquis Vente d\'arme Illégal pour 7500 ~y~LystyLifeCoins\n~w~Faites un ticket sur Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter ***Vente d\'arme Illégal***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [7500] LystyLifeCoins', 56108)    
    end, function()
        return
    end)
end)
...
AddEventHandler('aBoutique:vehunique', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    OnProcessCheckout(source, 7500, string.format("Achat Véhicule Unique"), function()
        xPlayer.showNotification('~r~LystyLife~w~~n~Vous avez acquis Véhicule Unique pour 7500 ~y~LystyLifeCoins\n~w~Faites un ticket sur le Discord Boutique')
...
AddEventHandler('ewen:buyweapon', function(weapon, price, label)
    ...
    OnProcessCheckout(xPlayer.source, SecurityWeapons[weapon].price, string.format("Achat de : %s", label), function()
        ...
        xPlayer.showNotification("~r~LystyLife~w~~n~Vous avez acheter : " .. SecurityWeapons[weapon].label .. " sur la boutique !")
    end, function()
        xPlayer.showNotification("~r~Vous ne posséder pas les points nécessaires")
        return
    end)
...
RegisterNetEvent('LystyLife:process_checkout_case')
AddEventHandler('LystyLife:process_checkout_case', function(type)
    local source = source;
    if (source) then
        local identifier = GetIdentifiers(source);
        local xPlayer = ESX.GetPlayerFromId(source)
            ...
            OnProcessCheckout(source, MysteryBox.ListBox[type].price, "Achat d'une caisse ("..MysteryBox.ListBox[type].label..')', function()
                ...
                local giveReward = {
                    ...
                    ['vehunique'] = function() 
                        ...
                        xPlayer.showNotification('~r~LystyLife ~w~~n~Faites un ticket boutique pour obtenir votre véhicule Unique')
                    end,
                    ["helico"] = function(_s, license, player)
                        ...
                        MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE (`vehicle` LIKE @hash AND `type` = 'aircraft' AND `owner` = @owner) ", {
                            ...
                            if resultVeh[1] then
                                xPlayer.showNotification('~r~LystyLife~w~~n~Vous aviez déjà le véhicule que vous avez gagner\nUne caisse vous à été rendu')
                    ...
                    ["weapon"] = function(_s, license, player)
                        if (ILALARMEOUPAS(xPlayer, GetHashKey(result))) then
                            xPlayer.showNotification('~r~LystyLife~w~~n~Vous aviez déjà l\'arme que vous avez gagner\nUne caisse vous à été rendu')
                            xPlayer.addInventoryItem(type, 1)
                        else
    ...
    if type == 'gold' then
        ...
        if (identifier['fivem']) then
            ...
            OnProcessCheckout(source, 1000, string.format("Achat de : VIP GOLD 1 Mois"), function()
                ...
                xPlayer.showNotification("~r~LystyLife~w~~n~Vous avez acheter : VIP ~y~GOLD ~w~(1 mois) sur la boutique !")
    ...
    elseif type == 'diamond' then
        ...
        if (identifier['fivem']) then
            ...
            OnProcessCheckout(source, 2000, string.format("Achat de : VIP Diamond 1 Mois"), function()
                ...
                xPlayer.showNotification("~r~LystyLife~w~~n~Vous avez acheter : VIP ~r~DIAMOND ~w~(1 mois) sur la boutique !")
                SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter un ***VIP DIAMOND (1 mois)***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [2000] LystyLifeCoins', 56108)
            end, function()
                return
            end)
        end
...
Citizen.CreateThread(function()
    ...
    for k,v in pairs(MysteryBox.ListBox) do
        ESX.RegisterUsableItem(k, function(source)
            ...
            TriggerClientEvent('closeall', source)
            Wait(100)
            TriggerEvent('LystyLife:process_checkout_case_item', source, k, MysteryBox.ListBox[k].label)
        end)
    end
end)

RegisterNetEvent('LystyLife:process_checkout_case_item')
AddEventHandler('LystyLife:process_checkout_case_item', function(src, type, label)
    if (src) then
...
ESX.RegisterUsableItem('jetoncustom', function(source)
    ...
    if veh~= 0 then 
        ...
        xPlayer.showNotification('~r~LystyLife ~w~~n~Vous devez être a l\'intérieur d\'un véhicule pour le customiser')
    end
end)
