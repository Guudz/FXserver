...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == 'mecano' ) or not cfg_custom.IsMecanoJobOnly and not lsMenuIsShowed then
				isInLSMarker = false
			...
			if IsControlJustReleased(0, 38) and not lsMenuIsShowed and isInLSMarker then
				if (PlayerData.job and (PlayerData.job.name == 'mecano' )) or not cfg_custom.IsMecanoJobOnly then
					--vehPedIsIn = GetVehiclePedIsIn(playerPed, false)
					terminatePurchase()
					getCarPrice()
					vehicleClass = GetVehicleClass(vehPedIsIn)
					--print("vehicleClass: " .. vehicleClass)
					if ((vehicleClass ~= 8 or not cfg_custom.IsMotorCycleBikerOnly) and PlayerData.job.name == 'mecano') then
