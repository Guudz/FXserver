...
AddEventHandler('Recru:benny', function()
	...
	if xPlayer.job.name == 'mecano' then
		for i=1, #xPlayers, 1 do
			local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Benny\'s', '~r~Annonce', '~y~Recrutement en cours, rendez-vous au Benny\'s', 'CHAR_CARSITE3', 8)
		end
	end
end)
...
ESX.RegisterUsableItem('fixkit', function(source)
	...
	TriggerClientEvent('esx:showNotification', source, "Vous venez d'utilisé un ~b~~h~ Kit de réparation !")   
end)
...
ESX.RegisterUsableItem('carokit', function(source)
	...
	TriggerClientEvent('esx:showNotification', source, "Vous venez d'utilisé un ~b~~h~ Kit de réparation Carrosserie !")    
end)
